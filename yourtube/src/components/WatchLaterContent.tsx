...
export default function WatchLaterContent() {
  ...
  const loadWatchLater = async () => {
    if (!user) return;
    try {
      const watchLaterData = await api.get(`/watch/${user._id}`);
  ...
  const handleRemoveFromWatchLater = async (item: any) => {
    if (!item.videoid?._id) return;
    try {
      await api.post(`/watch/${item.videoid._id}`);
      setWatchLater(watchLater.filter((w) => w._id !== item._id));
    } catch (error) {
          ...
          <div key={item._id} className="flex gap-4 group">
            ...
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                <DropdownMenuItem onClick={() => handleRemoveFromWatchLater(item)}>
